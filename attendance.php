<?php
	if(isset($_POST['faculty'])){
		...
		if($query->num_rows > 0){
			...
			if($status == 'in'){
				$sql = "SELECT * FROM attendance WHERE faculty_qr = '$id' AND date = '$date_now' AND time_in IS NOT NULL";
				$query = $conn->query($sql);
				if($query->num_rows > 0){
					$output['error'] = true;
					...
					//
					$sql = "INSERT INTO attendance (faculty_qr, date, time_in, status) VALUES ('$id', '$date_now', NOW(), '$logstatus')";
					if($conn->query($sql)){
				...
				$sql = "SELECT *, attendance.id AS uid FROM attendance LEFT JOIN faculty ON faculty.id=attendance.faculty_qr WHERE attendance.faculty_qr = '$id' AND date = '$date_now'";
				$query = $conn->query($sql);
				if($query->num_rows < 1){
					$output['error'] = true;
